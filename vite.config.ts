    ...
    server: {
      ...
      proxy: {
        ...
        '^/krakend': {
          ...
          target: 'http://rims.iptime.org:9092', // 사용할 요청 도메인을 설정한다.
        ...
        '^/countries': {
          // 프록시가 적용될 요청 경로의 시작 부분. 클라이언트가 보낸 요청의 URL이 api로 시작되면 이 설정이 적용된다.
          target: 'http://192.168.90.106:9091', // 사용할 요청 도메인을 설정한다.
          changeOrigin: true // HTTP 요청 헤더의 Host 값을 서버의 호스트와 일치하도록 변경한다. 이를 통해 클라이언트의 요청을 target에 설정된 도메인에서 온 것 처럼 변경할 수 있다.
        },
        '^/timezone': {
          // 프록시가 적용될 요청 경로의 시작 부분. 클라이언트가 보낸 요청의 URL이 api로 시작되면 이 설정이 적용된다.
        ...
        '^/api/v1/ws': {
          target: 'http://192.168.90.106:9000', // 사용할 요청 도메인을 설정한다.
          changeOrigin: true,
          ws: true
        },
        '^/svc-seer': {
          target: 'http://192.168.90.106:32055', // 사용할 요청 도메인을 설정한다.
          changeOrigin: true
        },
        '^/voyageMedia': {
          // 프록시가 적용될 요청 경로의 시작 부분. 클라이언트가 보낸 요청의 URL이 api로 시작되면 이 설정이 적용된다.
        ...
        '^/geotarget': {
          // 프록시가 적용될 요청 경로의 시작 부분. 클라이언트가 보낸 요청의 URL이 api로 시작되면 이 설정이 적용된다.
          target: 'http://192.168.90.106:7080', // 사용할 요청 도메인을 설정한다.
          changeOrigin: true, // HTTP 요청 헤더의 Host 값을 서버의 호스트와 일치하도록 변경한다. 이를 통해 클라이언트의 요청을 target에 설정된 도메인에서 온 것 처럼 변경할 수 있다.
        ...
        '^/VITE_SETTING_TARGET': {
          // 프록시가 적용될 요청 경로의 시작 부분. 클라이언트가 보낸 요청의 URL이 api로 시작되면 이 설정이 적용된다.
          target: 'http://rims.iptime.org:32100', // 사용할 요청 도메인을 설정한다.
          changeOrigin: true, // HTTP 요청 헤더의 Host 값을 서버의 호스트와 일치하도록 변경한다. 이를 통해 클라이언트의 요청을 target에 설정된 도메인에서 온 것 처럼 변경할 수 있다.
        ...
        // '^/VITE_LESSON_TARGET': {
        //   // 프록시가 적용될 요청 경로의 시작 부분. 클라이언트가 보낸 요청의 URL이 api로 시작되면 이 설정이 적용된다.
        //   target: 'http://rims.iptime.org:4600/api', // 사용할 요청 도메인을 설정한다.
        //   changeOrigin: true // HTTP 요청 헤더의 Host 값을 서버의 호스트와 일치하도록 변경한다. 이를 통해 클라이언트의 요청을 target에 설정된 도메인에서 온 것 처럼 변경할 수 있다.
        //   // rewrite: (path) => path.replace(/^\/VITE_SETTING_TARGET/, '') // 프록시 요청의 경로를 재작성하는 함수를 설정한다.
        // }
        '/rims-api': {
          target: 'http://rims.iptime.org:4500',
          changeOrigin: true
        },
        '/api': {
        ...
        '/kdt-api': {
          target: 'http://rims.iptime.org:4700',
          changeOrigin: true
        },
        '/unity': {
          target: 'http://rims.iptime.org:4700',
        ...
        '/dbdata': {
          target: 'http://rims.iptime.org:4800',
          changeOrigin: true
        },
        '/analysis-api': {
          target: 'http://rims.iptime.org:4800',
    ...
    plugins: [
      ...
      federation({
        ...
        remotes: {
          ...
          'svc-lesson': `http://rims.iptime.org:4500/assets/remoteEntry.js`,
          'svc-xinnos': `http://rims.iptime.org:4600/assets/remoteEntry.js`,
          'svc-kdt': `http://rims.iptime.org:4700/assets/remoteEntry.js`,
          'svc-vegas': `http://rims.iptime.org:4800/assets/remoteEntry.js`,
          'svc-mrc': `http://rims.iptime.org:4900/assets/remoteEntry.js`
        },
